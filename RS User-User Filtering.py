...
sigmas = [0]*n
movies = [ () for _ in range(n) ]

for i in range(n):
  movies_i = user2movie[i] #All films that were rated by user
  movies_i_set = set(movies_i) #for intersection check
...
for i in range(n):
  ...
  for j in range(n):
    if i < j:
      common_movies = (movies[i] & movies[j])
      if len(common_movies) > limit:
        num = sum(deviations[i][m]*deviations[j][m] for m in common_movies)
        if num != 0:
...
X_test = np.array([test.user_idx, test.movie_idx]).T
Y_test = np.array([test.rating]).T
X_train = np.array([train.user_idx, train.movie_idx]).T
...
def predict(X, Y, k):
  ...
  for i in range(X.shape[0]):
    ...
    num = 0
    den = 0
    for w, j in neighbors[cur_user]:
      try:
        num += w*deviations[j][cur_movie]
        den += abs(w)
      except KeyError:
        pass #We may have neighbors, that did not watch specific movie. Therefore
        #deviations[j][cur_movie] will throw error. We
    if den == 0:
       averages[cur_user]
    else:
      P[i] = num / den + averages[cur_user]
      P[i] = min(5, P[i])
      P[i] = max(0.5, P[i])
  return P
