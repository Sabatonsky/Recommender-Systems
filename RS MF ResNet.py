...
class MatrixFactorization(torch.nn.Module):
  def __init__(self, n, m, k, h):
    super().__init__()
    self.w = torch.nn.Embedding(n, k)
    self.u = torch.nn.Embedding(m, k)
    self.b = torch.nn.Embedding(n, 1)
    self.c = torch.nn.Embedding(m, 1)
    self.l1 = nn.Linear(2*k, h)
    self.l2 = nn.Linear(h, 1)

  def forward(self, user, movie):
    main = self.b(user) + self.c(movie)
    main += (self.w(user) * self.u(movie)).sum(dim=1, keepdim=True)
    main = main.squeeze()

    side = torch.cat((self.w(user), self.u(movie)), 1)
    side = self.l1(side)
    side = torch.relu(side)
    side = self.l2(side).flatten()
...
h = 10
num_epochs = 25
model = MatrixFactorization(n, m, k, h)
criterion = RMSELoss()
